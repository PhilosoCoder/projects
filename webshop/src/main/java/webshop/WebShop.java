...
public class WebShop {
    ...
    public Customer getCustomerWithMaxTotalAmount() {
        ...
        Customer customerWithMaxTotalAmount = orders.get(0).getCustomer();
        Order actualOrder = orders.get(0);
        for (Order order : orders) {
            if (order.getTotalAmount() > actualOrder.getTotalAmount()) {
                customerWithMaxTotalAmount = order.getCustomer();
                actualOrder = order;
            }
        }
        return customerWithMaxTotalAmount;
    }

    public List<Order> listOrdersSortedByTotalAmounts() {
        List<Order> result = new ArrayList<>(orders);
        result.sort(new OrderTotalAmountComparator());
        return result;
    }

    public List<Order> listOrdersSortedByDate() {
        List<Order> result = new ArrayList<>(orders);
        result.sort(new OrderDateComparator());
        return result;
    }
    ...
    private boolean checkIfCustomerCanBeVip(Customer customer) {
        int orderOfCustomer = 0;
        for (Order order : orders) {
            if (order.getCustomer().equals(customer)) {
                orderOfCustomer++;
            }
        }
        return orderOfCustomer > 4;
    }
